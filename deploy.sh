...
printf "\n==================================="
printf "\nBuilding image: mijoco/uasu_api."
printf "\nConnecting to: $SERVER on ssh port $PORT"
printf "\nDeploying container: $DOCKER_NAME."
printf "\nDeploying target: $TARGET_ENV."
printf "\n\n"

$DOCKER_CMD build -t mijoco/uasu_api .
$DOCKER_CMD tag mijoco/uasu_api mijoco/uasu_api
$DOCKER_CMD push mijoco/uasu_api:latest

  ssh -i ~/.ssh/id_rsa -p $PORT bob@$SERVER "
    cd /home/bob/hubgit/uasu-api
    pwd
    cat .env;
    docker login;
    docker pull mijoco/uasu_api;
